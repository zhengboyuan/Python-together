...
# 绘制饼图
    ...
    usrer_investment = st.sidebar.selectbox('用户是否自投', ['是','否'],index=1)
    st.subheader('用户自投参考')
    usrer_investment_empty = st.empty()
    
    if usrer_investment == "是":
        # st.subheader('用户自投参考')
        with usrer_investment_empty:
            container1_col, container2_col = st.columns(2)
            with container1_col:
                container1 = st.container(border=True,height=600)
                with container1:
                    container1.markdown('**用户软件、硬件投资情况**')
                    col_soft,col_hard_ware_first,col_hard_ware = st.columns(3)
                    soft_ware_investment = container1.number_input("软件投资(万元)", min_value=0,value=0, step=10,help='首年软件投资')
                    hard_ware_first = container1.number_input("硬件首年投资", min_value=0,value=0, step=1)
                    hard_ware = container1.number_input("后续每年投资", min_value=0,value=0, step=1,help = '包括每年软件、硬件、人员投入等……')
                    software_profit_share = container1.selectbox('是否软件平台利润分成', ['是','否'],index=1)
                    total_revenue['投资金额(软件+硬件)'] = 0
                    total_revenue.iloc[0,1] = soft_ware_investment + hard_ware_first
                    total_revenue.iloc[1:,1] = hard_ware
                    invest_ment_sum = total_revenue['投资金额(软件+硬件)'].sum()
                    total_revenue['投资金额累计'] = total_revenue['投资金额(软件+硬件)'].cumsum()
                    total_revenue['虚拟电厂总收益(万元/年)累计'] = total_revenue['虚拟电厂总收益(万元/年)'].cumsum()
                    if software_profit_share =='是':
                        software_profit_percent = container1.select_slider("与软件平台分成比例（%）", value=10,options=range(0, 101,5) )/100
                        total_revenue['软件平台利润分成'] = total_revenue['虚拟电厂总收益(万元/年)累计']*software_profit_percent
                        total_revenue['虚拟电厂累计净收益(万元/年)'] = total_revenue['虚拟电厂总收益(万元/年)累计'] - total_revenue['投资金额累计'] - total_revenue['软件平台利润分成'] 
                    else:
                        total_revenue['虚拟电厂累计净收益(万元/年)'] = total_revenue['虚拟电厂总收益(万元/年)累计'] - total_revenue['投资金额累计']
                    container1.markdown(':grey[***注：【用户自投】主要考虑为<u>软件初始投入、硬件投入、后续每年运维费用，其他开支可以计入【后续每年投资】中。</u>***]', unsafe_allow_html=True)
                    # container1.markdown("Hello, <u>World!</u> :sunglasses:", unsafe_allow_html=True)
                try:
                    break_even_index = total_revenue[total_revenue['虚拟电厂累计净收益(万元/年)'] > 0].idxmin().values.min()
                except ValueError:
                    break_even_index = None
                
                #st.write(break_even_index)
            with container2_col:
                container2 = st.container(border=True,height=600)
                with container2:
                    container2.markdown('**用户虚拟电厂10年累计投资、收益分布（万元/年）**')
                    col_12,col_13,col_14 = st.columns(3)
                    col_12.metric(label="总投资（万元）",value= invest_ment_sum,help='投资金额累计（软件+硬件+硬件后续每年投资）')
                    col_13.metric(label="总收益（万元）",value= round(total_revenue['虚拟电厂总收益(万元/年)'].sum(),2))
                    if break_even_index is not None:
                        col_14.metric(label="收益回报分析", value=break_even_index, delta="年收回")
                    else:
                        col_14.metric(label="收益回报分析", value="尚未达到盈亏平衡", delta="")
            
                    container2.bar_chart(total_revenue,y=["投资金额累计", "虚拟电厂总收益(万元/年)累计",'虚拟电厂累计净收益(万元/年)'] 
                                 ,stack=False)
                    container2.markdown(':red[*注：【虚拟电厂累计净收益】为正时开始盈利。*]')
            
            # 假设数据
            # 假设数据
            years = total_revenue.index
            investment = round(total_revenue['投资金额累计'],)
            revenue = round(total_revenue['虚拟电厂总收益(万元/年)累计'],)
            cost = round(total_revenue['虚拟电厂累计净收益(万元/年)'],) # 假设的另一个柱状图数据
            ...
            # # 在第一个轴上绘制第一个柱状图
            # ax1.bar(years - 0.2, investment, width=0.4, color='b', alpha=0.6, label='投资金额累计')
            # ax1.bar(years + 0.2, revenue, width=0.4, color='g', alpha=0.6, label='虚拟电厂总收益(万元/年)累计')
            
            # # 设置第一个轴的标签和标题
            # ax1.set_xlabel('年')
            # ax1.set_ylabel('投资金额累计&虚拟电厂总收益累计', color='b')
            ...
            # ax2 = ax1.twinx()
            # # 在第二个轴上绘制折线图
            # ax2.plot(years, cost, color='r', marker='+', label='虚拟电厂累计净收益(万元/年)')
            # # # 获取成本数据中的最小值，用于确定 y 轴的下限
            # min_cost = cost.min()
            # max_cost = cost.max()
            # y_min = min(0, min_cost * 1.1)
            # y_max = max(0, max_cost * 1.1)
            # # # #设置 y 轴的范围
            # ax2.set_ylim(y_min, y_max)
            # ax2.tick_params('y', colors='r')
            # ax2.legend(loc='upper right')
            # ax2.set_ylabel('虚拟电厂累计净收益', color='r')
            # # 在折线图的每个点旁边显示数据
            # for i, year in enumerate(years):  # 使用years的索引
            #     ax2.annotate(str(cost.loc[year])+str('万元'), (year, cost.loc[year]), textcoords="offset points", xytext=(0,10), ha='center')
            #     # ax2.annotate(str(revenue.loc[year]), (year, revenue.loc[year]), textcoords="offset points", xytext=(0,1), ha='center')
            #     # ax2.annotate(str(investment.loc[year]), (year, investment.loc[year]), textcoords="offset points", xytext=(0,1), ha='center')
        
            # # 设置标题和图例
            # plt.title('用户虚拟电厂10年累计投资、收益分布（万元/年）')
            #usrer_investment_empty.dataframe(total_revenue.T, width=2100)
        # # 使用st.pyplot()来显示图表
        # st.pyplot(fig)
        st.markdown('**收益明细数据：**')
        st.dataframe(total_revenue.T, width=2100)
    else:
        usrer_investment_empty.empty()
    st.markdown('### 附：参考依据')

    st.markdown('#### 1、湖北省辅助服务次数')
    st.markdown("[湖北省辅助服务参与次数依据](https://mp.weixin.qq.com/s/KKLsqehSEkGcaua5NGg4Ug)")
    st.markdown('''> :grey[以湖北省为例，<u>2024年4月至7月已开展填谷调峰调用近25次</u>，
                为新能源消纳提供有力支撑；上海、浙江、深圳等地利用电动汽车、换电站、5G基站等优质调节资源，
                开展了虚拟电厂二次调频辅助服务能力验证，为虚拟电厂常态化参与调频市场奠定基础。]'''
                , unsafe_allow_html=True)
    st.markdown('#### 2、湖北省辅助政策文件')
    st.markdown("[湖北源网荷储电力调峰辅助服务市场运营规则](https://hzj.nea.gov.cn/xxgk/zcfg/202401/t20240125_230773.html)")
    st.markdown("""
| 辅助服务类型 | 非保供时期（申报价格上限（元/兆瓦时）） | 保供时期（申报价格上限（元/兆瓦时）） |
| ------------ | ---------- | -------- |
| 削峰         | M7=1000    | M8=2000  |
| 填谷         | M9=400     | M9=400   |
