...
def display_sidebar():
    logo_url = "https://pic.imgdb.cn/item/667f7317d9c307b7e9d790bb.jpg"
    st.logo(logo_url)
    ...
    if energy_storage_option == "是":
       ...
       energy_storage_deep = st.sidebar.number_input("放电深度（%）",min_value=0,max_value=100, value=90, step=1)/100
       ...
       Response_Ratio = st.sidebar.number_input("储能响应比例（%）", min_value=0,max_value=100,value=80, step=1)/100
       ...
       split_ratio = st.sidebar.number_input("与用户分享比例（%）", value=50, step=1,min_value=0,max_value=100)/100
    ...
    if controllable_load_energy_storage_option == "是":
        ...
        controllable_load_up_ratio = st.sidebar.number_input("负荷增长率（%）", value=5, step=1,min_value=0,max_value=100)/100
        controllable_load_response_ratio= st.sidebar.number_input("负荷响应比例（%）", value=13, step=1,min_value=0,max_value=100)/100
        ...
        controllable_load_split_ratio = st.sidebar.number_input("可调可控负荷与用户分享比例（%）", value=50, step=1,min_value=0,max_value=100)/100
